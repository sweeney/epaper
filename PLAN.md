...
func (c *Canvas) StrokeRect(r image.Rectangle, ink epaper.Ink)
func (c *Canvas) Line(a, b image.Point, ink epaper.Ink)
func (c *Canvas) Ellipse(r image.Rectangle, ink epaper.Ink)
func (c *Canvas) StrokeEllipse(r image.Rectangle, ink epaper.Ink)
func (c *Canvas) Polygon(pts []image.Point, ink epaper.Ink)   // convex
...
1. **Conformance, byte-exact, no hardware.** Reproduce
   `testdata/conformance/conformance.idx` and `.bin` exactly. Three checks in
   order (render, pack, end-to-end) per `testdata/README.md` §2.

   The fixture is font-free *and* free of PIL's `ImageDraw` — every shape comes
   from an integer algorithm specified in `testdata/README.md`, for the reason
   in §9.8. `.bin` was packed by the vendor library on our board, which is what
   makes it an oracle for the packing; the *pixels* are an oracle for our own
   specified rules, which is a different and weaker claim, deliberately.

   Already demonstrated achievable: a throwaway Go implementation written from
   those rules reproduces all 120,000 pixels with zero mismatches.
...
8. ~~**Band 6 of the conformance fixture cannot be reproduced from the
   documented rules.**~~ **Resolved: fixture regenerated from portable rules.**

   Found at M1, by re-deriving the fixture from `testdata/README.md`. Bands 1–5
   reproduced exactly; band 6 did not, because it was drawn with PIL's
   `ImageDraw.ellipse`, `.line` and `.polygon`. Those rasterisers are exactly as
   unportable as glyph hinting — the same reason the pattern has no text in it —
   so M8's argument that the fixture is byte-comparable *because it is font-free*
   was incomplete. Underneath it, §4.5's `Canvas` had no `Ellipse` or `Polygon`
   at all, so the acceptance test was asking for something the API did not offer.

   **Taken: option (b).** `tools/conformance.py` now imports no `ImageDraw`.
   Every shape is drawn from an integer algorithm — inscribed-ellipse inside
   test, Bresenham lines, half-space convex fill — written out in
   `testdata/README.md` under *the exact geometry rules*. The fixture was
   regenerated on the Pi against the live vendor library on 2026-09-14, so
   `conformance.bin` remains a genuine vendor-produced packing oracle.

   The reasoning, for the record: the packing rule and the Bayer matrix are
   genuinely vendor-defined, so the vendor is the right oracle for them. Ellipse
   rasterisation is not vendor-defined by anything, so inheriting PIL's would
   have pinned our public API to another project's internals forever. *Measure,
   don't inherit* (§1.3) cuts the same way here.

   Verified four ways:

   | Check | Result |
   |---|---|
   | Only band 6 changed | 174 px differ from the old fixture, all at `y >= 260`; bands 1–5 byte-identical |
   | Cross-machine determinism | Mac (Pillow 12.1.1) and Pi (Pillow 12.3.0) produce identical `.idx` |
   | Packing still holds | re-packing `.idx` reproduces `.bin` exactly, 30,000 bytes |
   | **Portability, the actual claim** | a **Go** implementation written from the README rules reproduces all **120,000 px with zero mismatches**, band 6 included |

   That last row is the one that mattered — it makes M8's conformance check
   achievable end to end, rather than achievable for five bands out of six.

   Consequence: §4.5 gains `StrokeRect`, `Ellipse`, `StrokeEllipse` and
   `Polygon`. `Polygon` is documented convex-only; the half-space fill is not a
   general scanline rasteriser, and nothing needs one yet.
